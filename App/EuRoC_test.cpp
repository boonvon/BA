...
using std::string;
using std::vector;
DEFINE_string(imgs_folder, "", "The folder containing l and r folders, and the calib.yaml");//包含l和r文件夹的文件夹，以及calib.yaml
DEFINE_int32(grid_row_num, 1, "Number of rows of detection grids");//检测网格的行数
DEFINE_int32(grid_col_num, 1, "Number of cols of detection grids");//检测网格的列数
DEFINE_int32(max_num_per_grid, 70, "Max number of points per grid");//每格最大点数
DEFINE_double(feat_quality, 0.07, "Tomasi-Shi feature quality level");//Tomasi-Shi特征质量等级
DEFINE_double(feat_min_dis, 10, "Tomasi-Shi feature minimal distance");//Tomasi-Shi具有最小距离
DEFINE_bool(not_use_fast, false, "Whether or not use FAST");//是否使用FAST 
DEFINE_int32(pyra_level, 2, "Total pyramid levels");//总金字塔等级
DEFINE_int32(start_idx, 0, "The image index of the first detection (from 0)");//第一次检测的图像索引（从0开始）
DEFINE_int32(end_idx, -1, "The image index of the last detection");//最后一次检测的图像索引
DEFINE_double(uniform_radius, 40, "< 5 disables uniformaty enforcement");//<5禁用统一执法
DEFINE_int32(ft_len, 125, "The feature track length threshold when dropout kicks in");//辍学时的特征轨道长度阈值
DEFINE_double(ft_droprate, 0.05, "The drop out rate when a feature track exceeds ft_len");//当特征轨道超过ft_len时的退出率
DEFINE_bool(show_feat_only, false, "wether or not show detection results only");//只有或者不显示检测结果
DEFINE_int32(fast_thresh, 10, "FAST feature threshold (only meaningful if use_fast=true)");//FAST特征阈值（仅当use_fast = true时才有意义”
DEFINE_double(min_feature_distance_over_baseline_ratio,
              4, "Used for slave image feature detection");//用于右图像图像特征检测”
DEFINE_double(max_feature_distance_over_baseline_ratio,
              3000, "Used for slave image feature detection");//用于右图像特征检测”）;
DEFINE_string(iba_param_path, "", "iba parameters path");// 参数路径
DEFINE_string(gba_camera_save_path, "", "Save the camera states to when finished");//将相机状态保存到完成时
DEFINE_bool(stereo, false, "monocular or stereo mode");//单眼或者双目模式
DEFINE_bool(save_feature, false, "Save features to .dat file");//“将功能保存到.dat文件
...
void load_asl_calib(const std::string &asl_path,
  ...
  calib_param.Camera.cameraK_lr[1] << v_float[0], 0, v_float[2],
      0, v_float[1], v_float[3],
      0, 0, 1;
  // distortion_coefficients 畸变矫正系数
...
bool create_iba_frame(const vector<cv::KeyPoint>& kps_l,
  ...
  IBA::CurrentFrame& CF = *ptrCF;  //当前帧CF
  IBA::KeyFrame& KF = *ptrKF; //关键帧KF
  ...
  if (!need_new_kf) {
    KF.iFrm = -1;
    //  to make it deterministic使其具有确定性
    InitPOD(KF.C);
    InitPOD(KF.d);
  } else {
    KF.iFrm = CF.iFrm;
    KF.C = CF.C.C;
    // MapPointMeasurement, duplication复制 of CF
    KF.zs = CF.zs;
    // MapPoint
...
int main(int argc, char** argv) {
  ...
  //std::vector<string> &rimg_name  保存左右图像的名字
  ...
  for (int i=0; i<limg_name.size(); i++) {
    ...
    img_file_paths.push_back(l_png);//完整的路径+左图像名字（左图索引）
    slave_img_file_paths.push_back(FLAGS_imgs_folder + "/mav0/cam1/data/" + rimg_name[i]);//完整的路径+右图像名字（右图索引）
    iba_dat_file_paths.push_back(FLAGS_imgs_folder + "/dat/" + limg_name[i] + ".dat");//完整的IMU数据索引
  }
  if (!FLAGS_stereo) {
    slave_img_file_paths.clear();  //如果测试单目想过则删除右图像
  }
  ...
  XP::DuoCalibParam duo_calib_param;  //容错
  try {
    load_asl_calib(FLAGS_imgs_folder, duo_calib_param);
  } catch (...){
    LOG(ERROR) << "Load calibration file error";  //加载校准文件错误
    return -1;
  }
  // Create masks based on FOVs computed from intrinsics内参//相机相关参数
  std::vector<cv::Mat_<uchar> > masks(2);
  for (int lr = 0; lr < 2; ++lr) {
    float fov;
    if (XP::generate_cam_mask(duo_calib_param.Camera.cv_camK_lr[lr],  //相机内参
                              duo_calib_param.Camera.cv_dist_coeff_lr[lr],  //相机畸变系数
                              duo_calib_param.Camera.img_size,    //图像尺寸
                              &masks[lr],  //???
                              &fov)) {  //???
  ...
  // Adjust end image index for detection调整最终图像索引以进行检测
  ...
  for (int it_img = FLAGS_start_idx; it_img < FLAGS_end_idx; ++it_img) {
    ...
    // get timestamp from image file name (s)  获取时间戳
    ...
    // load slave image保存右图像
    cv::Mat slave_img_smooth;  // for visualization later用于以后的可视化
    ...
    // 获取prev_time_stamp和time_stamp中的imu测量值以计算old_R_new  （计算旋转矩阵）
    imu_meas.reserve(10);
    ...
    // use optical flow  from the 1st frame使用第一帧的光流
    if (it_img != FLAGS_start_idx) {
      ...
      if (imu_meas.size() > 1) {
        ...
        // 在这里，我们只是转换链到旋转，并假设零转换
        cv::Matx33f old_R_new;
        XP::XpQuaternion I_new_q_I_old;  // The rotation between the new {I} and old {I}  q（四元数方式表示）
      ...
      feat_track_detector.update_img_pyramids(); //图像金字塔
      ...
      feat_track_detector.detect(img_in_smooth,
                                 ...
                                 &key_pnts,
                                 &orb_feat);
      feat_track_detector.build_img_pyramids(img_in_smooth, //构建图像金字塔
                                             XP::FeatureTrackDetector::BUILD_TO_PREV);
    }
    if (slave_img_smooth.rows > 0) {
      CHECK(orb_feat_slave.empty());
      auto det_slave_img_start = std::chrono::high_resolution_clock::now();  //计时间
...
//========================================================
  ...
  // for comparsion with asl groundtruth  作比较
