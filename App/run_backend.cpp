...
bool RunSolver(const Configurator &cfgor, const InputSequence &IS, const std::string param = "",
  ...
  const int nFrms = IS.Size();//帧数×××××××××××××××××××××××××需要更改
  if (nFrms == 0) {
    return false;
  }
...
//定义求解器
  ...
  solver.Create(IS.m_K, serial, verbose, debug, history, param, IS.m_dir/*=======＝＝＝＝＝＝===============================步骤１创建优化器*/
#ifdef CFG_GROUND_TRUTH
              , IS.m_XsGT.empty() ? NULL : &IS.m_XsGT
              , IS.m_dsGT.empty() ? NULL : &IS.m_dsGT
#endif
              );
#ifndef WIN32//串行，并行工作？  还可以加入GPU的吗？
...
//#if 1
...
#endif
  // IBA::Error eLBA, eGBA;   局部BA以及全局BA的误差
  ...
  const AlignedVector<Camera> &CsGT = IS.m_CTGT.m_Cs;//****************************************需要更改
...
#endif
    IS.LoadCurrentFrame(iFrm, &CF, &KF, &solver);//******************************************需要更改
#ifdef CFG_DEBUG
...
// #if 1
...
#endif
    if (IS.LoadRelativeConstraint(iFrm, &Z)) {//*********************************************需要更改
#ifdef CFG_GROUND_TRUTH
    ...
    if (IS.LoadDeleteKeyFrames(iFrm, &iFrms)) {//*********************************************需要更改
    ...
    if (IS.LoadDeleteMapPoints(iFrm, &idxs)) {//*********************************************需要更改
      solver.DeleteMapPoints(idxs);
    }
    //UT::Print("[%d] %f\n", iFrm, Timer::GetTime() - t1);
    if (IS.LoadUpdateCameras(iFrm, &iFrms, &Cs)) {//*********************************************需要更改
...
//#if 1
    ...
    if (dt < IS.m_dt) {//*********************************************需要更改
      const int ms = static_cast<int>((IS.m_dt - dt) * 1000.0 + 0.5);//*********************************************需要更改
#ifdef WIN32
...
bool RunConfig(const std::string &fileName) {
  Configurator cfgor(fileName);  //文件操作类
  IBA::Time tLBA, tGBA;          //优化类数据结构中的时间戳数据结构（时间戳+n）struct Time {  float t;  int n;};
  const std::string dir = cfgor.GetArgument("input_directory_all");  //文件操作
  if (dir == "") {
...
// #ifdef CFG_DEBUG
  ...
  // Google的开源库，解析命令行参数，一般都放在 main 函数中开始位置
  // 终端命令行运行的时候，对命令行参数进行解析
  google::ParseCommandLineFlags(&argc, &argv, false);
#endif
  // TODO(mingyu): Upgrade to use gflag once it's ready on Windows
  // 一旦在Windows上准备好，就升级为使用gflag，前面的日志兼容是在Ubuntu下面的日志，后面则是Ubuntu和win都要运行的程序。
  if (argc == 2) {
